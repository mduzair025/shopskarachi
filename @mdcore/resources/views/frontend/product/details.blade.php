                        ...
                        @if ($product_image_gallery && count($product_image_gallery))
                            ...
                            <div class="single-main-image">
                                @if(!empty($product->badge))
                                    <span class="sale">{{$product->badge}}</span>
                                @endif
                                <a href="{{ get_attachment_image_by_id($gallery_image, 'full')['img_url'] }}" class="long-img magnific" data-key="{{ $loop->iteration + 1 }}">
                                    <i class="las la-search-plus"></i>
                                </a>
                                <figure class="zoom zoom-js-handle"
                    ...
                    <div class="user-select-option">
                        ...
                        <div class="product_related_info mt-5">
                            @if($item_in_stock)
                                ...
                                <div class="text-success"><small>({{ $item_stock_count.' '.Str::of('item')->plural($item_stock_count). ' ' . __('available in stock') }})</small></div>
                            @else
                                <div class="text-secondary text-danger"><small>({{ __('Item is not available in stock') }})</small></div>
                            @endif
                        </div>
            ...
            <div class="col-lg-12">
                ...
                <div class="single-new-collection-item single-product-item">
                    ...
                    <div class="product-details-wrap">
                        <h3 class="product-title"><a href="{{ route('frontend.products.single', $product->slug) }}">{{ Str::limit($product->title, 23) }}</a></h3>
